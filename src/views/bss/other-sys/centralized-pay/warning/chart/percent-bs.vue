<template>
  <Card shadow>
    <p slot="title" class="analysis-card-title">各功能科目支付占比</p>
    <div class="percent-div">
      <Spin v-if="loading" fix large></Spin>
      <div v-if="$_.isEmpty(list)" class="no-data">暂无数据</div>
      <Row v-else :gutter="8" class="mt-8" justify="start" v-for="(item, index) in list" :key="`${item.bsName}-${index}`">
        <i-col :xs="24" :sm="24" :md="6" :lg="6">
          <span :class="{ 'click-avalible' : item.percent !== 0 }" style="font-size: 10px; color: #808695" @click="itemClick(item)">{{ item.bsName }}</span>
        </i-col>

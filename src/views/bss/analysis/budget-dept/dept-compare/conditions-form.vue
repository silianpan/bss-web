      ...
      <Row type="flex" :gutter="8">
        ...
        <i-col :xs="24" :sm="6" :md="6" :lg="6">
          <FormItem :label="`部门`" style="width:100%;">
            <Select v-model="formDatas.govDept1" transfer clearable filterable placeholder="部门1">
              <Option v-for="(item, index) in deptRange" :value="item" :key="index">{{ item }}</Option>
            </Select>
            <Select v-model="formDatas.govDept2" transfer clearable filterable placeholder="部门2">
              <Option v-for="(item, index) in deptRange" :value="item" :key="index">{{ item }}</Option>
            </Select>
          </FormItem>
        </i-col>
    ...
    async listDepts() {
      ...
      if (ret.ok()) {
        ...
        if (!this.$_.isEmpty(ret.data)) {
          this.$set(this.formDatas, 'govDept1', ret.data[0])
          if (ret.data.length > 1) {
            this.$set(this.formDatas, 'govDept2', ret.data[1])
          } else {
            this.$set(this.formDatas, 'govDept2', '')
          }
        } else {
          this.$set(this.formDatas, 'govDept1', '')
          this.$set(this.formDatas, 'govDept2', '')
        }
      }
    },

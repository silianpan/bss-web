    ...
    <div class="containerWrapper">
      ...
      <div class="rightWrapper">
        <div class="functionBar bottomLine">
          <div class="functionBarMain">
            <div class="leftGroup">
              <div class="formLabel">
                <ButtonGroup>
                  <Button v-has-per="['bss-budget-upload-btn']" type="primary">批量预处理</Button>
                </ButtonGroup>
              </div>
            </div>
            <div class="rightGroup">
              ...
              <div class="formLabel">
                <i-input v-model="conForm.searchVal" placeholder="输入名称" style="width:260px" class="searchCriminal" search @on-search="searchFile"></i-input>
              </div>
              <div class="formLabel">
                <Button @click="showModelToggle">{{
                  showModel ? '列表显示' : '图标显示'
                }}</Button>
              </div>
            </div>
          ...
          <template v-else>
            <div class="wrapperList" v-if="showModel">
              <template v-for="item in data">
                <div class="wrapperItem" :key="item.id">
                  <div class="wrapperItemMain" @click="showRowDetail(item)">
                    <div class="icon">
                      <i :class="['typeIcon', `ext-${item.extName}`]"></i>
                    </div>
                    <div class="name">
                      <Tooltip transfer max-width="300" :content="item.name">{{
                        item.name | filterSlice(20)
                      }}</Tooltip>
                    </div>
                  </div>
                </div>
        ...
        <div class="pageContainer" style="position">
          <Page :total="page.totalRecords" :current="page.pageNum" show-total show-sizer show-elevator :page-size="page.pageSize" @on-change="listChangePage"
            @on-page-size-change="listChangePageSize" />
        </div>
      </div>
    </div>
  </div>
...
<script>
import '@/views/bss/budget/style/budget-list.less'
...
import DetailInfo from '@/views/bss/budget/components/detail-info'
import PreHandle from './pre-handle'
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              'span',
              this.$_.isNil(params.row.deptName) ? '-' : params.row.deptName
            )
          }
        },
        {
  ...
  methods: {
      ...
      let drawer = DynamicDrawerUtil.newInstance(
        {
          ...
          width: 95,
          maskClosable: true,
          components: { PreHandle },
          render: (h) => {
            return <pre-handle data={data}></pre-handle>
          }
        },
        this
      )
    ...
    async initYearTree() {
      ...
      let treeData = !this.$_.isEmpty(yearRange)
        ? [{ title: '年份', expand: true, children: yearRange }]
        : []
      return treeData
    },
    async initAreaTree() {
    ...
    async initDeptTree(areaId) {
      ...
      let treeData = !this.$_.isEmpty(govDept)
        ? [{ title: '部门', expand: true, children: govDept }]
        : [{ title: '暂无部门数据', disabled: true }]
      return treeData
    },
    // 选择条件
    yearClick(arr, node) {
    ...
    async areaClick(arr, node) {
      this.conForm.dept = {}
      this.$set(
        this.conditions.deptTree,
        'datas',
        node.selected && this.$_.isNil(node.root)
          ? await this.initDeptTree(node.id)
          : []
      )
    ...
    changeLv(value) {
      const treeTitle =
        value === 'deptLv' || value === 'totalLv' ? '区域' : '本级'
      ...
      if (value === 'thisLv' || value === 'totalLv') {
        ...
        // 使用了v-if要等页面刷新完成
        if (
          !this.$_.isEmpty(this.conditions.deptTree.datas) &&
          this.deptShowFlag
        ) {
          this.$nextTick(() => {
      ...
      let drawer = DynamicDrawerUtil.newInstance(
        {
          ...
          render: (h) => {
            return <detail-info data={data}></detail-info>
          }
        },
        this
      )
